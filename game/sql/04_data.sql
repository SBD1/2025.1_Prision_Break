...
CALL criar_sala('Cela do Jogador', 'Um espaço apertado com uma cama, uma pia e as quatro paredes que te separam do mundo. É o único lugar para pensar sem olhares curiosos.', 0, FALSE);
CALL criar_sala('Cela do Irmão', 'Idêntica à sua, mas carrega o peso da urgência. É o motivo pelo qual os planos são traçados e os riscos são tomados.', 1, FALSE);
CALL criar_sala('Bloco A', 'Fileiras de celas que abrigam centenas de detentos. Território da gangue Mafia Abruzzi.', 5, FALSE);
CALL criar_sala('Bloco B', 'Fileiras de celas que abrigam centenas de detentos. Território da gangue La Familia.', 5, FALSE);
CALL criar_sala('Bloco C', 'Fileiras de celas que abrigam centenas de detentos. Território da gangue Irmandade Ariana.', 6, FALSE);
CALL criar_sala('Bloco D', 'Fileiras de celas que abrigam centenas de detentos. Território da gangue Os Justiceiros', 6, FALSE);
CALL criar_sala('Bloco E', 'Fileiras de celas que abrigam centenas de detentos. Território da gangue Os Justiceiros.', 5, FALSE);
CALL criar_sala('Solitária', 'Um cômodo escuro e vazio para onde os guardas enviam os problemáticos para quebrar seu espírito.', 8, TRUE);
CALL criar_sala('Banheiro', 'Úmido, sujo e com pouca privacidade. É um ponto de encontro para negócios escusos, trocas de informações e confrontos.', 3, FALSE);
CALL criar_sala('Biblioteca', 'Um refúgio de silêncio em meio ao caos. As estantes empoeiradas guardam conhecimento, códigos e segredos.', 2, FALSE);
CALL criar_sala('Patio Principal', 'Vasta área a céu aberto, cercada por muros altos. É onde os detentos se exercitam, socializam e conspiram.', 7, FALSE);
CALL criar_sala('Cozinha', 'O coração quente e barulhento da prisão. Fonte de ferramentas improvisadas e conflitos.', 4, FALSE);
CALL criar_sala('Refeitório', 'Um grande salão onde toda a população carcerária se reúne. Palco principal para demonstrações de poder.', 6, FALSE);
CALL criar_sala('Enfermaria', 'Um oásis de limpeza. O cheiro de antisséptico domina o ar. Local para tratar ferimentos e obter medicamentos.', 4, FALSE);
CALL criar_sala('Telhado da Enfermaria', 'Uma rota de fuga perigosa e exposta. Oferece uma visão panorâmica, mas também te transforma em um alvo fácil.', 9, TRUE);
CALL criar_sala('Copa dos Policiais', 'A área de descanso dos guardas. Um lugar para café, conversas e negócios corruptos.', 8, TRUE);
CALL criar_sala('Recepção', 'A primeira barreira entre a prisão e o mundo exterior. Área de alta segurança, com detectores de metal.', 8, FALSE);
CALL criar_sala('Diretoria', 'O escritório do Diretor. Símbolo máximo de autoridade. É aqui que os segredos da administração estão trancados.', 10, TRUE);
CALL criar_sala('Sala de Câmeras', 'O centro nervoso da segurança. Monitores exibem cada corredor e cada cela.', 9, TRUE);
CALL criar_sala('Entrada', 'O portão principal. A última barreira de aço e concreto entre você e a liberdade. Fortemente guardado.', 9, TRUE);
CALL criar_sala('Área externa leste', 'Ao emergir no telhado, você é recebido pelo abraço do céu aberto. O som distante dos grilos substitui o zumbido das luzes.', 0, TRUE);
CALL criar_sala('Área externa sul', 'Ao passar pelo último portão de aço, o ar da noite muda completamente. O cheiro de terra molhada e grama fresca domina.', 0, TRUE);
CALL criar_sala('Oficina', 'O som de metal contra metal preenche o ar. Fonte inestimável de materiais brutos, ferramentas e potenciais armas.', 5, FALSE);
CALL criar_sala('Corredor da Cela do Irmão', 'Um corredor estreito e mais silencioso, terminando na cela que guarda o motivo de sua luta.', 2, FALSE);
CALL criar_sala('Corredor do Bloco A', 'O som das grades e das conversas é constante. Este corredor serve como fronteira do território da Mafia Abruzzi.', 4, FALSE);
CALL criar_sala('Corredor do Bloco B', 'O som das grades e das conversas é constante. Este corredor serve como fronteira do território de La Familia.', 4, FALSE);
CALL criar_sala('Corredor da Ala Norte', 'O principal eixo da área residencial da prisão, conectando os diversos blocos de celas.', 3, FALSE);
CALL criar_sala('Corredor do Bloco D', 'O som das grades e das conversas é constante. Este corredor serve como fronteira do território de La Familia.', 4, FALSE);
CALL criar_sala('Corredor do Bloco E', 'O som das grades e das conversas é constante. Este corredor serve como fronteira do território dos Justiceiros.', 4, FALSE);
CALL criar_sala('Corredor longo', 'Um trecho monótono e retilíneo de concreto, vigiado por câmeras em ambas as extremidades.', 5, FALSE);
CALL criar_sala('Corredor da Solitária', 'O caminho para o inferno. Um corredor frio e mal iluminado que leva à sala mais temida da prisão.', 7, FALSE);
CALL criar_sala('Corredor da Ala Oeste', 'A artéria que leva ao barulho da oficina e do refeitório. Frequentemente patrulhado.', 4, FALSE);
CALL criar_sala('Corredor da Ala Leste', 'Conduz ao silêncio da biblioteca e ao cheiro de antisséptico da enfermaria. É uma rota mais calma, mas não menos vigiada.', 3, FALSE);
CALL criar_sala('Corredor da Ala Sul', 'O limiar do poder. Este corredor dá acesso à recepção e à entrada principal. A presença de guardas aqui é muito mais intensa.', 7, TRUE);
CALL criar_sala('Corredor dos guardas', 'Território proibido. Limpo, bem iluminado e silencioso, conecta áreas vitais de segurança.', 8, TRUE);
...
CALL criar_agente_penitenciario(1, 'Paul Kellerman', 6, 6, 0, true, 20, 'Agente Federal');
CALL criar_agente_penitenciario(3, 'Donald Self', 3, 3, 0, true, 10, 'Agente Federal');
CALL criar_agente_penitenciario(18, 'Warden Pope', 10, 10, 0, false, NULL, 'Diretor');
CALL criar_agente_penitenciario(14, 'Sara Tancredi', 0, 0, 0, false, NULL, 'Médica');
CALL criar_agente_penitenciario(25, 'Guarda Miller', 5, 5, 5, false, NULL, 'Carcereiro');
CALL criar_agente_penitenciario(28, 'Guarda Evans', 5, 5, 5, false, NULL, 'Carcereiro');
CALL criar_agente_penitenciario(12, 'Guarda Wright', 6, 6, 6, false, NULL, 'Supervisor do Refeitório');
CALL criar_agente_penitenciario(19, 'Guarda Davis', 5, 5, 5, false, NULL, 'Supervisor da Oficina');
CALL criar_agente_penitenciario(16, 'Guarda Peterson', 5, 5, 5, false, NULL, 'Recepcionista');
CALL criar_agente_penitenciario(35, 'Guarda Roy', 7, 7, 7, false, NULL, 'Patrulha Administrativa');
CALL criar_agente_penitenciario(32, 'Guarda Baker', 6, 6, 0, true, 15, 'Carcereiro');
CALL criar_agente_penitenciario(31, 'Guarda Stokes', 7, 7, 0, false, NULL, 'Guarda da Solitária');
CALL criar_agente_penitenciario(11, 'Guarda Hayes', 5, 5, 0, false, NULL, 'Patrulha da Cozinha');
CALL criar_agente_penitenciario(33, 'Guarda Wallace', 6, 6, 0, false, NULL, 'Patrulha da Ala Leste');
CALL criar_agente_penitenciario(13, 'Guarda Fisher', 5, 5, 0, false, NULL, 'Vigilância do Pátio');
CALL criar_agente_penitenciario(34, 'Guarda O''Malley', 6, 6, 0, false, NULL, 'Patrulha da Ala Sul');

CALL criar_prisioneiro('Michael Scofield', 2, 8, 100, 'Assalto a banco (planejado para entrar na prisão)', 'Os Fox River Eight');
CALL criar_prisioneiro('John Abruzzi', 3, 6, 90, 'Homicídio e chefia em organização criminosa', 'Mafia Abruzzi');
CALL criar_prisioneiro('Theodore "T-Bag" Bagwell', 5, 5, 80, 'Estupro, sequestro e assassinato', 'Irmandade Ariana');
CALL criar_prisioneiro('Fernando Sucre', 6, 7, 95, 'Roubo à mão armada', 'Os Fox River Eight');
CALL criar_prisioneiro('Benjamin "C-Note" Franklin', 7, 8, 93, 'Tráfico e deserção militar', 'Os Justiceiros');
CALL criar_prisioneiro('David "Tweener" Apolskis', 6, 7, 85, 'Roubo e desacato', 'Os Fox River Eight');
CALL criar_prisioneiro('Carlos Ortega', 4, 6, 88, 'Tráfico de drogas e agressão', 'La Familia');
CALL criar_prisioneiro('Lincoln Burrows (O Irmão)', 2, 6, 100, 'Acusado de assassinato', 'Os Fox River Eight');
CALL criar_prisioneiro('Hector', 13, 6, 90, 'Tráfico e extorsão', 'La Familia');
CALL criar_prisioneiro('Eduardo (Líder)', 4, 5, 100, 'Liderança de gangue', 'La Familia');
CALL criar_prisioneiro('Samuel, o Velho', 10, 3, 80, 'Contrabando (múltiplas sentenças)', 'Mafia Abruzzi');
CALL criar_prisioneiro('Javier, o Informante', 8, 5, 85, 'Roubo e falsificação', 'La Familia');
CALL criar_prisioneiro('Silas, o Isolado', 8, 7, 100, 'Agressão grave a um guarda', 'Irmandade Ariana');
CALL criar_prisioneiro('Charles, o Intelectual', 10, 4, 80, 'Fraude e lavagem de dinheiro', 'Os Fox River Eight');
